...
class SeapadReferralWalletAdapter extends seapad_ref_func_1.SeaPadReferralFunc {
    ...
    async upsertReferral(types, args, optionTx, gasBudget, packageObjectId) {
        const message = this._SeaPadReferralInput.upsertReferral(types, args, optionTx, gasBudget, packageObjectId);
        return await this._walletContextState.signAndExecuteTransactionBlock(this.buildTx(message));
    }
    async removeReferral(types, args, optionTx, gasBudget, packageObjectId) {
        const message = this._SeaPadReferralInput.removeReferral(types, args, optionTx, gasBudget, packageObjectId);
        return await this._walletContextState.signAndExecuteTransactionBlock(this.buildTx(message));
    }
